...
import com.leanplum.internal.uieditor.LeanplumListenerSwizzler;
...
public class LeanplumActivityHelper {
   ...
   */
  private static void applyInterfaceEdits(Activity activity) {
    try {
      LeanplumViewManager.setIsUpdateInProgress(true);
      LeanplumListenerSwizzler.addOnGlobalChangeListener(activity);
      // Directly apply UI updates once active development mode is disabled
      LeanplumViewManager.applyUpdateRules(activity, VarCache.getUpdateRuleDiffs(), false, false,
          null, null);
      LeanplumViewManager.applyUpdateRules(activity, VarCache.getEventRuleDiffs(), true, false,
          null, null);
