...
public class Newsfeed {
  ...
  private Newsfeed() {
    ...
    this.didLoad = false;
    this.newsfeedChangedHandlers = new ArrayList<>();
  }
  ...
  void removeMessage(String messageId) {
    ...
    Request req = Request.post(Constants.Methods.DELETE_NEWSFEED_MESSAGE, params);
    req.send();
  }
  ...
  void load() {
    ...
    Map<String, NewsfeedMessage> messages = new HashMap<>();
    if (newsfeed == null) {
  ...
  void downloadMessages() {
    ...
    Request req = Request.post(Constants.Methods.GET_NEWSFEED_MESSAGES, null);
    req.onResponse(new Request.ResponseCallback() {
      ...
      public void response(JSONObject responses) {
        try {
          ...
          if (response == null) {
            Log.e("No newsfeed response received from the server.");
            return;
          }

          JSONObject messagesDict = response.optJSONObject(Constants.Keys.NEWSFEED_MESSAGES);
          if (messagesDict == null) {
            Log.e("No newsfeed messages found in the response from the server.", response);
            return;
          }
          int unreadCount = 0;
  ...
  public List<NewsfeedMessage> allMessages() {
    List<NewsfeedMessage> messages = new ArrayList<>();
    try {
  ...
  public List<NewsfeedMessage> unreadMessages() {
    List<NewsfeedMessage> unreadMessages = new ArrayList<>();
  ...
  /**
   * Returns the newsfeed messages associated with the given getMessageId identifier.
   */
