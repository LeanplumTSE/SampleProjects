...
public class Log {
  ...
  public static void logLargeString(String string) {
    if (string == null) {
      return;
    }
    ...
    for (int i = 0; i < length; i += MAX_LENGTH) {
      if (i + MAX_LENGTH < length) {
        log(LeanplumLogType.PRIVATE, string.substring(i, i + MAX_LENGTH));
      } else {
        log(LeanplumLogType.PRIVATE, string.substring(i, length));
      }
    }
  }
