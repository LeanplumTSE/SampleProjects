...
class AESCrypt {
  ...
  private static enum EncryptionType {
     ...
     * <p>
     * With the exception of LEGACY_TOKEN ciphertexts, which must continue to be supported,
     * we will use the first two characters to determine the encryption protocol.
     */
    APP_ID_KEY(1);
  ...
  public String encrypt(String plaintext) {
    ...
    if (cipherText.isEmpty() || cipherText.equals(plaintext) || !cipherText.startsWith("[")) {
      Log.w("Leanplum", "Invalid ciphertext: " + cipherText);
      return null;
    }
  ...
  /**
   * Decrypts a ciphertext in either legacy or current format, using a password based on
   *   context parameters.
  ...
  public String decrypt(String cipherText) {
    ...
    if (encryptionSpec == null) {
      Util.verboseLog("Got null encryptionSpec for encrypted: " + cipherText);
    } else {
      switch (encryptionSpec.first) {
        case LEGACY_TOKEN:
          if (token == null || token.isEmpty()) {
            Log.e("Leanplum", "decrypt called with null token");
          } else {
            ...
            if (result != null && parseCiphertextInternal(result) != null) {
              Log.e("Leanplum", "Discarding legacy value that appears to be an encrypted value: " +
                  result);
              return null;
            }
          }
        ...
        case APP_ID_KEY:
          if (appId == null || appId.isEmpty()) {
            Log.e("Leanplum", "decrypt called with null appId");
          } else {
    ...
    if (result == null) {
      Log.w("Leanplum", "Unable to decrypt " + cipherText);
    }
    return result;
  }
   ...
   */
  // VisibleForTesting
  static String encryptInternal(String password, String plaintext) {
    try {
      return Arrays.toString(performCryptOperation(Cipher.ENCRYPT_MODE, password,
          plaintext.getBytes("UTF-8")));
    } catch (UnsupportedEncodingException e) {
      Log.w("Leanplum", "Unable to encrypt " + plaintext, e);
      return null;
    }
  }
   ...
   *
   * @param ciphertext
   *          Must be a valid byte array represented as a string as returned by
   *          Arrays.toString().
   */
  ...
  /**
   * Performs either an encryption or a decryption based on the mode. In case of
   * exception, returns null.
   * 
   * @param mode
   *          Should be either Cipher.ENCRYPT_MODE or Cipher.DECRYPT_MODE
   * @param password
